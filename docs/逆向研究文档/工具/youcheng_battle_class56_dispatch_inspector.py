...
# -*- coding: utf-8 -*-
"""
《幽城幻剑录》Battle FIRTTECH class 5 / class 6 可达路径静态调查器。

这个脚本服务于 Godot 兼容引擎研究固化56。它只读取输入文件，绝不改写 RPG.exe、
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="验证《幽城幻剑录》Battle FIRTTECH class5/class6 可达计算路径与 class6 下游抑制边界。"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    overall_pass = exe_pass and bool(resource_report["PASS"]) and bool(machine_report["PASS"]) and bool(conclusion_report["PASS"])
    ...
    report: Dict[str, object] = {
        ...
        "工具版本": TOOL_VERSION,
        "固化节点": 56,
        "目标": "闭合 FIRTTECH class5/class6 可达计算分派、0x42C770假专用分支边界，以及class6两处已证明下游抑制。",
        "输入": {
    ...
    print(f"[固化56] 总体结果：{'PASS' if overall_pass else 'FAIL'}")
    print(f"[固化56] JSON：{args.json_out}")
    print(f"[固化56] CSV ：{args.csv_out}")
