...
/* 以十进制输出 32 位无符号整数；TOML 的毫秒值用十进制日志更容易直接阅读。 */
 ...
 * - 这样用户把 TOML 写坏时只会失去自定义设置，不会影响 Hook 安全。
 */
...
/*
 * 向 Runtime 声明一个精确 E8 rel32 CALL；此时仅登记，提交整批事务时才写入。
 * Runtime 验证原版目标或已知的同签名链；无 Runtime 或无正在构建的事务时返回失败。
 */
int Runtime_PatchCall(u32 call_address, u32 expected_target, const void* replacement, const char* label);

/*
 * 保留安装失败分支的兼容入口。声明阶段尚未写码，实际撤销由 AbortSdkHookTransaction 完成。
 * 此函数只确认 Runtime 已接管，不会单独改写 CALL。
 */
...
/* RuntimeHost 把两个 DisplayRebuild CALL 与 Bink 指针先声明，全部通过后一次提交。 */
