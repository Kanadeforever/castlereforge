...
def main():
    ...
    parser.add_argument("--ddraw", type=Path, help="可选：将真实兼容层复制到临时目录做启动生命周期验证，不修改源DLL")
    parser.add_argument("--runtime-dll", type=Path, help="可选：复制当前Runtime构建产物以验证真实调用链")
    ...
    with tempfile.TemporaryDirectory(prefix="castle_input_test_") as folder:
        if args.ddraw:
            # DLL会自行保存配置，所以只加载新临时目录里的副本，绝不从只读参考目录直接执行。
            copied_ddraw = Path(folder) / "ddraw.dll"
            shutil.copy2(args.ddraw.resolve(), copied_ddraw)
            environment["CASTLE_TEST_DDRAW_DLL"] = str(copied_ddraw)
        if args.runtime_dll:
            copied_runtime = Path(folder) / "Castle_Runtime.dll"
            shutil.copy2(args.runtime_dll.resolve(), copied_runtime)
            environment["CASTLE_TEST_RUNTIME_DLL"] = str(copied_runtime)
        ...
        subprocess.run([str(Path(folder)/"test.exe")], cwd=folder, env=environment, check=True, timeout=30)
