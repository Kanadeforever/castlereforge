> **2026-09-20 v0.4.3 当前候选：** 有手柄启动默认普通手柄所有权；实体鼠标同tick优先，
> 明确数字键/左杆出沿可回切，普通右杆漂移忽略。合法范围按住RB开启持续快捷层，ABXY/上下左右
> 可连续新按，只响应快捷；未绑定不回落到原功能。松RB后共用64ms容错，旧动作保护到自身释放。
> 战斗只绑定六项，完全待机未新增菜单；不再要求每次快捷后松RB。鼠标接线改为固定调用点链，
> 已用真实cnc-ddraw验证建窗重挂后仍有效。须与Widescreen v0.12.3及Runtime 0.1.1成套替换。
...
1. 普通槽 `111b` 仍默认“存档”，上下顺序保持存档→读档→取消。
2. 禁用存档的 `011b` 首帧直接高亮“读档”，上下只在读档/取消之间移动。
3. 测试 `101b/010b/000b`、动态禁用和恢复，确认跳过、边界、不循环与无强制焦点行为。
4. 排队后按钮变disabled时清旧pending；恢复后必须重新按确定键，不能补发。
5. 从槽位打开三项窗口的同一确定按下沿不能立即执行默认动作。
6. 实体鼠标/Back/RT接管和重新使用方向键均正常。
7. 标题、天书、地图存档点、二次Yes/No和分页完整回归。
8. R43剧情多选、剧情RT、Back常驻、Battle Target隐藏及全部旧菜单快速回归。
