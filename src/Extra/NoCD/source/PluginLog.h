...
// 四个正式 ASI 共用的极小日志工具。
//
// 官方 ASI 从本版起必须由 Castle_Runtime.dll 承载，因此这里只保留“把业务文字交给
// Runtime Log 服务”这一条路径。日志目录、UTF-8 BOM、并发锁、刷新和关闭都由 Runtime
// 统一负责；插件本身不再持有文件句柄，也不会在 mods\asi 中散落旁路日志。
// ============================================================================
...
// 进程退出时只清空跨模块指针；真实文件句柄始终由已固定驻留的 Runtime 持有。
inline void Close() {
    gRuntimeLogApi = nullptr;
    gRuntimeLogPlugin = 0u;
}

// 旧调用点仍传入模块和日志名，保持插件内部接口稳定；文件名由 Runtime 根据 ASI 自动生成。
