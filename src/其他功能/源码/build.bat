...
echo [1/4] BUGFix.asi - 已合并稳定 CrashFix test2 双调用路径修复
cl.exe %CFLAGS% "%SRC_DIR%\BUGFix.cpp" %LFLAGS% /OUT:"%OUT_DIR%\BUGFix.asi"
if errorlevel 1 goto :fail
call :check_pe "%OUT_DIR%\BUGFix.asi"
if errorlevel 1 goto :fail

echo [2/4] AnytimeSave.asi - v0.3.1a Safe Fallback Save
cl.exe %CFLAGS% "%SRC_DIR%\AnytimeSave.cpp" %LFLAGS% /OUT:"%OUT_DIR%\AnytimeSave.asi"
if errorlevel 1 goto :fail
call :check_pe "%OUT_DIR%\AnytimeSave.asi"
if errorlevel 1 goto :fail

echo [3/4] NoCD.asi
cl.exe %CFLAGS% "%SRC_DIR%\NoCD.cpp" %LFLAGS% /OUT:"%OUT_DIR%\NoCD.asi"
if errorlevel 1 goto :fail
call :check_pe "%OUT_DIR%\NoCD.asi"
if errorlevel 1 goto :fail

echo [4/4] MaxGrowthAndDrop.asi
cl.exe %CFLAGS% "%SRC_DIR%\MaxGrowthAndDrop.cpp" %LFLAGS% /OUT:"%OUT_DIR%\MaxGrowthAndDrop.asi"
if errorlevel 1 goto :fail
