...
`build.bat` 仍以 x86 clang-cl/MSVC ABI、`/W4 /WX /utf-8 /GS- /Zl /nodefaultlib` 逐一编译 29 个 C 文件。部署只需 ASI、INI 和兼容 x86 SDL3.dll；不得打包 RPG.exe 或构建中间件。

R38 已经把 LT 调查从 `left_stick_latched` 改成不回中的连续状态机，但实机确认仍有两个手感问题：方向不够跟手，并且某些摇杆方向不能稳定选中画面上对应的互动目标。R39 不再把第一次推杆时的鼠标位置当轮盘中心，而是从原版 resolver 快照取得当前角色 `actor+0x10/+0x14` 的屏幕位置作为中心；目标在摇杆前方 180° 内按无方向偏差的整数角度评分自然分区，不再使用 ±63.4°硬候选锥；迟滞收紧到约 2～3°。

R38 的震动修复已经用户确认：普通隐藏鼠标 hover 可互动对象不再震动。该逻辑在 R39 冻结；LT 调查、Back 常驻鼠标、自由地图 RT 临时鼠标仍只在原版 resolver 首次确认新的可互动 hover 时短震，真正回普通手柄模式的高优先级反馈保持。

机器验证：29/29 C 严格编译 PASS；两套 `/timestamp:0` 链接逐字节一致；ASI 181,248 bytes，SHA-256 `a49a89c20fea24f2fe53ed048341e23734718e67b1a6ef7fde7b85803b1dc160`；无 EXE 75/0、可信原版 Oracle 96/0。实机仍需重点验证一圈连续旋转的方向覆盖和切换跟手度。
